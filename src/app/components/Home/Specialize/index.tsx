...
const Specialize = () => {
  const valeurAjoutee = [
    ...
    'Orientation éclairée vers les métiers de demain',
    'Stages + compétitions pour révéler l\'esprit entrepreneurial',
  ]

  const apportEtablissements = [
    'Programme innovant & clé en main',
    'Attractivité augmentée pour attirer élèves et parents',
    'Orientation scolaire structurée et alignée aux métiers du futur',
    'Participations à des compétitions d\'IA',
    'Stages en entreprises/startups',
    'Expérience pratique et immersion professionnelle',
  ]

  return (
    <>
      {/* Section 1: Notre Valeur Ajoutée */}
      ...
      {/* Section 2: Ce que nous apportons aux établissements */}
      <section className='py-24 lg:py-32 bg-gradient-to-b from-transparent via-secondary/5 to-secondary/10 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950'>
        <div className='container mx-auto max-w-6xl px-4 sm:px-6 lg:px-8'>
          ...
          >
            ...
            >
              <h2 className='text-3xl sm:text-4xl lg:text-5xl font-bold leading-tight text-[#0A004B] dark:text-white mb-4'>
                Ce que nous apportons aux établissements
              </h2>
            ...
            >
              {apportEtablissements.map((item, index) => (
                <motion.div
                  key={item}
                  variants={cardVariant}
